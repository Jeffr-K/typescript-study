...
함수 표현식은 사실 대부분 언어에서 언급되는 익명 함수의 다른 표현이다. 자바스크립트에서는 가끔 다음과 같은 형태로 작성된 코드를 만난다. 앞에서 살펴본 함수 표현식 개념이 없느 ㄴ상태에서 단순히 익명함수를 이름이 없는 함수로만 이해하면 이런 형태의 코드가 어떻게 도앚ㄱ하는지 그 원리를 쉽게 가늠하기 어렵다.
...
앞 코드를 이해하려면 먼저 연산자 우선순위를 고려해 코드를 분해해야 한다. 일반적으로 연산자들이 우선 순위가 다르면 (1 + 2) * 5 처럼 소괄호를 사용해 우선순위를 변경한다. 마찬가지로 함수 ㅗ출 연산자는 연산자의 우선순위가 매우 높으므로 함수 표현식 부분을 소괄호로 묶어서 컴파일러가 정상적으로 함수 표현식의 시작과 끝 부분을 알 수 있게 해야 한다.
...
그런데 흥미롭게도 중괄호 사용 여부에 따라 타입스크립트 문법이 동작하는 방식이 실행문 방시고가 표현문 방식으로 달라진다.
...
> 복합 실행문에서 변수의 유효 범위
> 
> 복합 실행문은 변수의 유효 범위도 지역범위로 제한한다. 따라서 다음처럼 두 함수의 몸통에 똑가틍ㄴ 이름의 변수가 있더라도 각 함수의 몸통에서만 유효하므로 서로 간섭하지 않늗나.
...
타입스크립트는 다음처럼 매개변수의 이름과 똑가틍ㄴ 이름의 속성을 가진 객체를 만들 수 있다. 이때 속성값 부분을 생략할 수 있는 단축구문을 제공한다.
